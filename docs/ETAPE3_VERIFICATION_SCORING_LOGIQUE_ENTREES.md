...
> **FIGÉE** — calibrée sur nos replays, elle domine le canon public ; la
> structure additive est saturée (le levier est dans les portes, pas les
> poids). Les évolutions passeront exclusivement par des portes de
> qualification (R1, R2, R6) ou de l'exécution (R7). Réouverture uniquement
> sur preuve ≥ 100 trades par bande montrant un composant au WR inversé.
...
- **Scoring** : plus calibré que le canon public (poids empiriques par asset) ;
  ...
  du volume d'espérance positive. R3 (entrée confirmation LTF) a été tué par la sonde : la confirmation
  avant fill n'existe que 0-3% du temps avec notre entrée TR-bord (fill à la
  première touche) — le concept canonique suppose une attente plus profonde
  dans la zone, architecture différente.
