// ════════════════════════════════════════════════════════════════════
//  SMC INSTITUTIONAL SCALPING v11 — Portage MQL5 (Phase 1 : indicateur)
//  Portage de smc_indicateur_v11.pine (Pine Script v6 → MQL5)
...
bool   g_isXAU, g_isXAG, g_isNAS, g_isBTC, g_isDAX, g_assetReconnu;
...
void InitAssetCalibration() {
    ...
    g_assetReconnu = g_isXAU || g_isXAG || g_isNAS || g_isBTC || g_isDAX;

    // Pine lignes 33-36
    g_pipValue = g_isXAU ? 0.1 : g_isXAG ? 0.01 : 1.0;
    g_pvLot    = g_isXAU ? 10.0 : g_isXAG ? 50.0 : g_isNAS ? 20.0 :
    ...
    g_autoTp3Mins = g_isXAU && g_tfM15 ? 60  : g_isXAU && g_tfH1 ? 240 :
                    ...
                    g_isNAS && g_tfM15 ? 30  : g_isNAS && g_tfH1 ? 120 :
    ...
    g_autoSlMode =
        ...
        (g_isXAU || g_isNAS || g_isDAX) ? "1× ATR sous OB" : "Bas OB";
}
...
void f_asianHlCompute(int i, const datetime &time[], const double &high[],
    ...
    // Invalidation si cassé (Pine 2680-2693, barstate.isconfirmed implicite via i = bar clôturé).
    // close[i] > _ahHighDrawn → Asian High invalidé ; close[i] < _ahLowDrawn → Asian Low invalidé.
    if(g_ahHighDrawn_valid && close[i] > g_ahHighDrawn) {
        g_ahHighDrawn = 0.0;
        g_ahHighDrawn_valid = false;
    }
    if(g_ahLowDrawn_valid && close[i] < g_ahLowDrawn) {
        g_ahLowDrawn = 0.0;
...
int SeuilMoyen()  { return g_isDAX ? 11 : g_isNAS ? 10 : g_isBTC ? 8 : 7; }
int SeuilFort()   { return (g_isXAG || g_isBTC) ? 99 : g_isDAX ? 16 : g_isNAS ? 15 : 10; }
int SeuilInstit() { return (g_isXAG || g_isBTC) ? 99 : g_isDAX ? 19 : g_isNAS ? 17 : 12; }
int ScoreMax()    { return g_isXAU ? 13 : g_isXAG ? 14 : g_isNAS ? 19 :
...
void f_createBuySignals(int i, const datetime &time[], const double &close[], double atr14) {
    ...
    for(int k = 0; k < g_nObBull; k++) {
        ...
        double sl = (g_autoSlMode == "1.5× ATR sous OB") ? rb - 1.5 * atr14 :
                    (g_autoSlMode == "1× ATR sous OB")   ? rb - atr14        :
                    (g_autoSlMode == "2× ATR sous OB")   ? rb - 2.0 * atr14  : rb;
        double r     = entry - sl;
        ...
        sl = entry - r;
        double tp1 = entry + r;   // Pine ligne 2882 : TP1 = entry + R (1R)
        ...
        if(m3 < 1e15 && m3 >= tp2) tp3 = m3;   // Pine correctif 1.3 : TP3 >= TP2
...
void f_createSellSignals(int i, const datetime &time[], const double &close[], double atr14) {
    ...
    for(int k = 0; k < g_nObBear; k++) {
        ...
        double sl = (g_autoSlMode == "1.5× ATR sous OB") ? rt + 1.5 * atr14 :
                    (g_autoSlMode == "1× ATR sous OB")   ? rt + atr14        :
                    (g_autoSlMode == "2× ATR sous OB")   ? rt + 2.0 * atr14  : rt;
        double r     = sl - entry;
        double slMax = SlMaxForAsset(atr14);
        double slMin = SlMinForAsset(atr14);
        ...
        r  = MathMax(slMin, MathMin(slMax, r));
        sl = entry + r;
        double tp1 = entry - r;   // Pine ligne 3101 : TP1 = entry - R (1R)
