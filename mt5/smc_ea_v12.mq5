// ════════════════════════════════════════════════════════════════════
//  SMC INSTITUTIONAL SCALPING v12 — EA backtestable (miroir Pine étalon + Rust)
//  Dérivé de smc_indicateur_v12.mq5 (indicateur) — miroir strict v12.
//  ROADMAP_EA_MT5.md Phase 0 : copie brute, déclaration indicator→expert.
// ════════════════════════════════════════════════════════════════════
#property copyright "Rono"
#property version   "12.00"
#property strict

// === SECTION TRADING (Phase 2) ===
#include <Trade/Trade.mqh>
CTrade g_trade;

// --- Diagnostic Phase 3 (temporaire) : compteurs pour identifier 0 trade ---
int g_diagSignalsCreated = 0;   // signaux créés par f_createBuy/SellSignals
int g_diagFilled         = 0;   // signaux fillés par le lifecycle
int g_diagOrdersSent     = 0;   // ordres envoyés au broker
int g_diagOrdersOk       = 0;   // ordres acceptés
int g_diagCloses         = 0;   // clôtures exécutées

// Inputs trading (backtest) — alignés sur le modèle de trade v11 (i_capital=10000, i_riskPct=1.0).
input double InpCapital = 10000.0;   // Capital initial ($)
input double InpRiskPct = 1.0;       // Risque par trade (% du capital)
input int    InpMagic   = 20260801;  // Magic number (filtre EA)
input bool   InpExecMarket = false;  // true = marché à la cassure, false = limite au bord OB (défaut Pine)
...
struct SignalT {       // signal BUY/SELL généré
    ...
    ulong     ticket;   // ticket MT5 de la position (0 = pas encore ouverte) — Phase 2 EA
    bool      orderTried; // true si un ordre a déjà été tenté (anti-retry, évite le flood de logs)
};
...
void f_drawBPR(const datetime &time[]) {
    ...
    for(int k = 0; k < g_nBpr; k++) {
        int idx = g_bpr[k].barIdx - 2;              // origine du gap récent (bar_index[2])
        if(idx < 0) idx = 0;
        if(idx >= rates_total) continue;
        datetime t0 = time[idx];
        color clr = g_bpr[k].dead ? clrDarkGray
                                  : (g_bpr[k].isBull ? COL_BPR_BULL : COL_BPR_BEAR);
        string nm = SMC_PREFIX + "BPR_" + IntegerToString(k);
        if(ObjectFind(0, nm) < 0) {
            ObjectCreate(0, nm, OBJ_RECTANGLE, 0, t0, g_bpr[k].top, tEnd, g_bpr[k].bot);
            ObjectSetInteger(0, nm, OBJPROP_FILL, true);
            ObjectSetInteger(0, nm, OBJPROP_BACK, false);
            ObjectSetInteger(0, nm, OBJPROP_SELECTABLE, false);
        }
        ObjectSetInteger(0, nm, OBJPROP_COLOR, clr);
        ObjectMove(0, nm, 0, t0, g_bpr[k].top);
        ObjectMove(0, nm, 1, tEnd, g_bpr[k].bot);
        // CE pointillée (mi-range).
        string nmC = nm + "_CE";
        if(ObjectFind(0, nmC) < 0) {
            ObjectCreate(0, nmC, OBJ_TREND, 0, t0, 0, tEnd, 0);
        ...
        double ce = (g_bpr[k].top + g_bpr[k].bot) / 2.0;
        ObjectSetInteger(0, nmC, OBJPROP_COLOR, clr);
        ObjectMove(0, nmC, 0, t0, ce);
        ObjectMove(0, nmC, 1, tEnd, ce);
        SmcNewLabel("BPR", tEnd, g_bpr[k].top, "BPR", clr, 0);
    }
    SmcFifo("BPR", MAX_VIS_LINES);
}
...
void f_createBuySignals(int i, const datetime &time[], const double &close[], double atr14) {
    ...
    for(int k = 0; k < g_nObBull; k++) {
        ...
        sig.ticket    = 0;   // Phase 2 EA : position MT5 pas encore ouverte
        sig.orderTried = false; // anti-retry
        ArrayResize(g_signals, g_nSignals + 1);
        g_signals[g_nSignals] = sig;
        g_nSignals++; g_diagSignalsCreated++;
...
// Affichage BSZones (pattern f_drawZones) : rectangle + label force.
...
#define PANEL_ROWS     36       // 6 en-têtes + 30 toggles (Score supprimé : toujours ON, clone Pine)
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam) {
    // EA backtest (Phase 1) : panneau de contrôle + fonds bgcolor retirés.
    // OnChartEvent est conservé (signature) mais vide — le Strategy Tester ne déclenche
    // pas d'événements clic/zoom utilisables.
}
...
int OnInit() {
    ...
    // EA backtest : pas de panneau de contrôle (clics inopérants en Strategy Tester).
    // On force TOUS les g_show* à true pour que la détection SMC tourne à fond,
    // indépendamment des GlobalVariables persistées (qui pourraient désactiver des modules).
    g_showStructure = g_showBOS = g_showMSS = g_showCHOCH = true;
    g_showLiq = g_showSweep = g_showFVG = g_showOB = g_showSignals = true;
    g_showLevels = g_showAsianHL = g_showZones = true;
    g_showBgTrend = g_showBgVol = g_showBgAtr = g_showBgPD = true;
    g_showBB = g_showPropulsion = g_showNDOG = g_showNWOG = true;
    g_showAsian = g_showLondon = g_showNY = g_showIB = true;
    g_showEqLn = g_showFIB = g_showH1 = g_showH4 = g_showW1 = g_showMN = true;
    Print("SMC EA v11 — asset=", _Symbol,
...
// ════════════════════════════════════════════════════════════════════════
// PHASE 2 — EXÉCUTION DES ORDRES (lifecycle interne Pine, exécution MT5)
// ════════════════════════════════════════════════════════════════════════
// Le lifecycle interne (f_updateTradeState) reste le cerveau : il décide les créations,
// fills, SL/BE/TP et clôtures. f_executeOrders se contente de traduire ces décisions
// en ordres MT5 réels pour que le Strategy Tester produise un rapport.

// Calcule la taille de lot à partir du risque (% du capital) et de la distance R.
// Utilise le SOLDE RÉEL du compte (AccountInfoDouble) au lieu de InpCapital figé.
double CalcLotFromRisk(double entry, double sl) {
    double rDist = MathAbs(entry - sl);
    if(rDist <= 0) return 0.0;
    double capital = AccountInfoDouble(ACCOUNT_BALANCE);
    if(capital <= 0) capital = InpCapital;
    double montantRisque = capital * InpRiskPct / 100.0;
    double lot = montantRisque / (rDist / g_pipValue * g_pvLot);
    // Normalisation aux bornes broker.
...
// Normalise le SL pour respecter la distance minimum du broker (SYMBOL_TRADE_STOPS_LEVEL).
// Retourne un SL valide (éloigné d'au moins stopsLevel du prix marché).
double NormalizeSl(bool isBull, double entry, double sl) {
    long stopsLevel = SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
    double minDist = stopsLevel * _Point;
    double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
    double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    if(isBull) {
        // BUY : SL sous l'entry. Distance min = bid - sl >= minDist.
        if(sl > bid - minDist) sl = bid - minDist;
    } else {
        // SELL : SL au-dessus de l'entry. Distance min = sl - ask >= minDist.
        if(sl < ask + minDist) sl = ask + minDist;
    }
    // Arrondi aux digits du symbole.
...
void f_executeOrders(const datetime &time[]) {
    ...
    for(int k = 0; k < g_nSignals; k++) {
        SignalT sig = g_signals[k];
        // --- OUVERTURE : signal créé, pas encore ouvert, ET fill confirmé par le lifecycle ---
        // Anti-retry : on marque le signal comme "tenté" via un champ dédié pour éviter
        // de re-soumettre à chaque bar si l'ordre échoue (sinon 16000+ logs inutiles).
        if(sig.ticket == 0 && sig.filled && !sig.closed && !sig.orderTried) {
            g_diagFilled++;
            double lot = CalcLotFromRisk(sig.entry, sig.sl);
            if(lot <= 0) { Print("[SMC diag] lot<=0 skip entry=", sig.entry, " sl=", sig.sl); g_signals[k].orderTried = true; continue; }
            double slNorm = NormalizeSl(sig.bull, sig.entry, sig.sl);
            g_diagOrdersSent++;
            if(sig.bull) g_trade.Buy (lot, _Symbol, 0.0, slNorm, 0.0, "SMC BUY");
            else         g_trade.Sell(lot, _Symbol, 0.0, slNorm, 0.0, "SMC SELL");
            uint rc = g_trade.ResultRetcode();
            if(rc == TRADE_RETCODE_DONE || rc == TRADE_RETCODE_PLACED || rc == TRADE_RETCODE_DONE_PARTIAL) {
                g_signals[k].ticket = g_trade.ResultOrder();
                g_diagOrdersOk++;
            } else {
                Print("[SMC diag] OrderSend ECHEC retcode=", rc, " (", g_trade.ResultRetcodeDescription(),
                      ") lot=", lot, " entry=", sig.entry, " sl=", sig.sl, " slNorm=", slNorm,
                      " stopsLevel=", SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL));
                g_signals[k].orderTried = true;   // ne pas re-tenter (évite le flood)
            }
        }
        // --- CLÔTURE : le lifecycle a marqué closed → on ferme la position MT5 ---
        if(sig.ticket != 0 && sig.closed) {
            g_diagCloses++;
            if(PositionSelectByTicket(sig.ticket)) {
                g_trade.PositionClose(sig.ticket);
            }
            sig.ticket = 0;
            g_signals[k].ticket = 0;
        }
    }
}

// OnTick remplace OnCalculate pour un EA (MT5 n'appelle OnCalculate QUE pour les indicateurs).
// On récupère les séries via CopyXxx (chronologique, index 0 = plus ancien) puis on rejoue la boucle.
void OnTick() {
    // Garde anti-O(n²) : OnTick est appelé à CHAQUE tick (57M en backtest "every tick").
    // On ne récupère les séries et ne recalcule QUE sur nouveau bar (ou 1er chargement).
    // Ce court-circuit AVANT les CopyXxx évite 57M × 6 appels réseau/cache inutiles.
    ...
    static datetime time[];  static double open[], high[], low[], close[];  static long tick_volume[];
    ...
    if(CopyTime(_Symbol, _Period, 0, rates_total, time) <= 0) return;
    if(CopyOpen(_Symbol, _Period, 0, rates_total, open) <= 0) return;
    if(CopyHigh(_Symbol, _Period, 0, rates_total, high) <= 0) return;
    if(CopyLow (_Symbol, _Period, 0, rates_total, low)  <= 0) return;
    if(CopyClose(_Symbol,_Period, 0, rates_total, close)<= 0) return;
    if(CopyTickVolume(_Symbol, _Period, 0, rates_total, tick_volume) <= 0) return;

    bool histReload = (g_prevCalc == 0) || (rates_total > g_prevRatesTotal + 1);
    if(!newBar && !histReload) return;
    ...
    // === Rejeu complet à chaque nouveau bar (comme l'indicateur original) ===
    // C'est plus lent que l'incrémental (~1h30 en every tick, moins en 1min OHLC), mais c'est
    // la SEULE méthode qui garantit un état SMC correct (le moteur n'est pas conçu pour l'incrémental).
    // La lenteur est acceptable pour un backtest de validation (1 passage sur 7 mois).
    ResetSmcState();
    ...
    for(int i = startBar; i <= rates_total - 2; i++) {
        ...
        // f_storeBg(i, rates_total, tick_volume, high, low, close, open, atr14);  // EA : affichage retiré

        // Génération signaux + lifecycle : le moteur fait vivre les trades virtuellement.
        ...
        // Exécution réelle : UNIQUEMENT sur le dernier bar clôturé (rates_total-2).
        // C'est le "présent" du backtest. Pendant le rejeu (i < rates_total-2), on n'exécute pas.
        if(i == rates_total - 2) f_executeOrders(time);
    }
    ...
    // f_drawFVG(time);  // EA backtest : affichage retiré (Phase 1)
    ...
    f_drawSignals(time);  // EA : visualisation trades conservée (historique + en cours)
    ...
    // --- Diagnostic Phase 3 (temporaire) : état final des compteurs (1 seule fois, dernier bar) ---
    static bool g_diagPrinted = false;
    if(!g_diagPrinted && (rates_total - g_prevCalc) <= 1) {
        Print("[SMC diag FINAL] signauxCrees=", g_diagSignalsCreated,
              " filled=", g_diagFilled,
              " ordresEnvoyes=", g_diagOrdersSent,
              " ordresOk=", g_diagOrdersOk,
              " closes=", g_diagCloses,
              " | g_nSignals=", g_nSignals,
              " assetReconnu=", g_assetReconnu);
        g_diagPrinted = true;
    }

    g_prevCalc = rates_total;
}
