...
pub async fn get_pics(
    ...
    let result = match &query.asset {
        Some(asset) => {
            db::straddle_pics::lister_recents_asset(pool, asset, 48, limit).await
        }
...
pub async fn get_feedback(
    ...
    match db::straddle_feedback::lister_recents_asset_categorie(pool, asset, categorie, limit)
        .await
    {
        Ok(rows) => HttpResponse::Ok().json(rows),
        Err(e) => HttpResponse::InternalServerError()
...
pub async fn cloturer_feedback(
    ...
    use sqlx::Row;
    let sig = sqlx::query(
        "SELECT prix_entree, stop_loss, cree_le FROM signaux WHERE id = ?",
    )
    .bind(&signal_id)
    .fetch_optional(pool)
    .await
