...
fn bougie(open: f64, high: f64, low: f64, close: f64) -> Candle {
    Candle { timestamp: Utc::now(), open, high, low, close, volume: 1000.0 }
}
...
fn test_suivi_option1_tp1_deplace_sl() {
    ...
    assert!((etat.sl_courant - entree).abs() < 1e-9,
        "Option 1 : SL attendu à {} (break-even), obtenu {}", entree, etat.sl_courant);
...
fn test_suivi_option2_pas_vente_partielle() {
    ...
    let etat = jouer_machine_etats(
        &bougies, sl, entree, &tps,
        true,
        false, // vente_partielle = Option 2
        "TEST", "LONG",
    ).expect("machine à états doit retourner un état");
    ...
    assert_eq!(etat.tps_done, vec!["tp1"]);
    assert!((etat.sl_courant - entree).abs() < 1e-9,
        "Option 2 : SL attendu à {} (break-even), obtenu {}", entree, etat.sl_courant);
...
fn test_tp2_deplace_sl_en_tp1() {
    ...
    let etat = jouer_machine_etats(
        &bougies, sl, entree, &tps, true, true, "TEST", "LONG",
    ).expect("état valide");

    assert_eq!(etat.tps_done, vec!["tp1", "tp2"]);
    assert!((etat.sl_courant - tps[0]).abs() < 1e-9,
        "Après TP2 : SL attendu en TP1 ({:.1}), obtenu {:.1}", tps[0], etat.sl_courant);
    assert!(etat.verdict.is_none());
}
