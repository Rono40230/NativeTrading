...
pub fn calculer_niveaux(r: &ScanResultat) -> NiveauxRocket {
    ...
    let tp1 = if r.hauteur_base > r.atr14 { r.prix + r.hauteur_base } else { r.prix + r.atr14 };
        ...
        match crate::ollama::rockets_filtre::filtrer_signal(&candidat, &historique).await {
            Ok(rep) => {
                tracing::info!(
                    ...
                    label_signal, r.ticker, rep.valide, rep.conviction
                );
                if !rep.valide || rep.conviction < CONVICTION_MIN {
                    tracing::info!(
                        "LLM rejette {} {} ({}/100): {}",
                        label_signal, r.ticker, rep.conviction, rep.raison
                    );
                    return;
                }
