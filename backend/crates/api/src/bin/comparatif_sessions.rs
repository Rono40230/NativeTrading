//! Comparatif Sessions H/L : bonus Module F ACTIF vs INACTIF — Phase 4
...
async fn executer() -> anyhow::Result<()> {
    ...
    let noms: [(&str, bool); 2] = [("SessHL ON", true), ("SessHL OFF", false)];
    ...
    lignes.push("COMPARATIF SESSIONS H/L : bonus Module F ON vs OFF — BE=Supprimé · TP3=DoL≤3R · BPR off (production)".into());
    ...
    lignes.push("─".repeat(100).into());
    lignes.push("Lecture : le delta R total (ON − OFF) = apport réel du bonus Sessions H/L.".into());
    lignes.push("Si ≤ 0 : le bonus est retiré du Pine étalon et du défaut Rust (règle replay).".into());

    let sortie = lignes.join("\n");
    println!("\n{}", sortie);
    std::fs::create_dir_all("data")?;
    std::fs::write("data/comparatif_sessions.txt", &sortie)?;
    println!("\n📄 Écrit : data/comparatif_sessions.txt");
    Ok(())
}
