//! Comparatif R2 — P/D Directionnel : porte ON vs OFF — étape 3
//! (docs/ETAPE3_VERIFICATION_SCORING_LOGIQUE_ENTREES.md, recommandation R2).
//!
//! Canon ICT : « never buy in premium, never sell in discount » — règle DURE
//! dans la méthodologie, +1 symbolique dans notre f_score. Porte testée à la
//! qualification v11 : trade bull interdit si close en premium, bear interdit
//! en discount (zone de tolérance équilibre = passe).
//!
//! Rejoue l'historique DB (production : BE=Supprimé, TP3=DoL≤3R, BPR et
//! sessions inactifs, mega-orders actif, sweep requis inactif) sous 2
//! branches :
//!   - P/D ON  : porte active (étalon Pine, branche testée).
//!   - P/D OFF : porte coupée (contre-factuel = production actuelle).
//!
//! Règle des 30 trades par cellule.
//!
//! Usage : cargo run --release -p api --bin comparatif_pd
//! Sortie : tableau console + data/comparatif_pd.txt
...
async fn executer() -> anyhow::Result<()> {
    ...
    let noms: [(&str, bool); 2] = [("P/D ON", true), ("P/D OFF", false)];
    ...
    for asset_id in ASSETS {
        ...
        for (tf_id, barres) in FENETRES {
            ...
            for (nom, pd_requis) in noms {
                let res = engine_v12::replay::rejouer_bougies_pd(
                    asset.clone(), tf, &bougies, true, amorce.clone(), pd_requis,
                );
    ...
    lignes.push("COMPARATIF R2 P/D DIRECTIONNEL : porte ON vs OFF — production (BE=Supp, TP3=DoL≤3R, mega on, sweep off)".into());
    ...
    lignes.push("─".repeat(100).into());
    lignes.push("Lecture : le delta R total (ON − OFF) = apport réel de la porte P/D directionnel.".into());
    lignes.push("Si ≤ 0 : le bonus est retiré du Pine étalon et du défaut Rust (règle replay).".into());

    let sortie = lignes.join("\n");
    println!("\n{}", sortie);
    std::fs::create_dir_all("data")?;
    std::fs::write("data/comparatif_pd.txt", &sortie)?;
    println!("\n📄 Écrit : data/comparatif_pd.txt");
    Ok(())
}
