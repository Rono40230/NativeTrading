//! Validation SP500 (Phase 5) : le mutisme (0 signal / 18 mois) venait de
//! l'absence du profil dans AssetCalibration/_assetReconnu (Pine + Rust).
//! Profil miroir NAS100 ajouté — CE binaire mesure la production de signaux
//! sur 24 mois × 3 TF (règle des 30 trades + WR/R attendus sains).
//!
//! Module F : les H/L des sessions Asie (MODULE 14) et Londres (MODULE 14b)
//! sont des pools BSL/SSL (ICT). Bonus f_score +2 si le prix est à ≤ 0.35×ATR
//! du H/L de session dans le sens du trade (état N-1, parité Pine).
//!
//! Rejoue l'historique DB (BE = Supprimé + TP3 = DoL≤3R = production, BPR
//! scoring inactif = production) sous 2 branches :
//!   - SessHL ON  : bonus actif (étalon Pine, production actuelle).
//!   - SessHL OFF : bonus coupé (contre-factuel).
//!
...
//!
//! Usage : cargo run --release -p api --bin comparatif_sessions
//! Sortie : tableau console + data/comparatif_sessions.txt
...
async fn executer() -> anyhow::Result<()> {
    ...
    let noms: [(&str, bool); 1] = [("SP500", false)];
    ...
    for asset_id in ASSETS {
        ...
        for (tf_id, barres) in FENETRES {
            ...
            for (nom, scoring_sessions) in noms {
                let res = engine_v12::replay::rejouer_bougies_sessions(
                    asset.clone(), tf, &bougies, true, amorce.clone(), scoring_sessions,
                );
    ...
    lignes.push("VALIDATION SP500 (Phase 5) — profil miroir NAS · BE=Supprimé · TP3=DoL≤3R (production)".into());
    ...
    for (nom, _) in noms {
        ...
        lignes.push(format!(
            "{:<10} TOTAL {:+.1}R · {} clôtures · R moyen {:+.3} · max DD {:.1}R",
            nom,
            total.r_total,
            total.fermes,
    ...
    lignes.push("─".repeat(100).into());
    lignes.push("Lecture : production de signaux SP500 avec le profil miroir NAS (règle 30 trades).".into());
    lignes.push("R moyen positif attendu ; calibration dédiée possible en étude future si justifiée.".into());

    let sortie = lignes.join("\n");
    println!("\n{}", sortie);
    std::fs::create_dir_all("data")?;
    std::fs::write("data/validation_spx.txt", &sortie)?;
    println!("\n📄 Écrit : data/validation_spx.txt");
    Ok(())
}
