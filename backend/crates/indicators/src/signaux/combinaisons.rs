...
fn bw_min(haute: &[f64], basse: &[f64], milieu: &[f64], i: usize, lookback: usize) -> f64 {
    (i.saturating_sub(lookback)..i)
        .filter_map(|j| {
            if milieu[j] == 0.0 || haute[j].is_nan() {
                None
            } else {
                Some((haute[j] - basse[j]) / milieu[j])
            }
        })
...
pub fn detecter_signaux_combines(
    ...
    for i in LOOKBACK..n {
        ...
        if let (Some(rsi), Some(bh), Some(bm), Some(bb)) =
            (rsi, boll_haute, boll_milieu, boll_basse)
        {
            if bh[i].is_finite() && bb[i].is_finite() && rsi[i].is_finite() {
                ...
                if c >= bh[i] && rsi[i] > 70.0 {
                    out.push(signal(
                        ...
                        NiveauForce::Fort,
                        format!(
                            "Bande haute + RSI overbought ({:.1}) — sell confirmé",
                            rsi[i]
                        ),
                        bh[i],
                        c,
                ...
                if ratio >= ATR_HAUSSE {
                    ...
                    if ml[i] > ms[i] && ml[i] > ml[i - 1] {
                        out.push(signal(
                            ...
                            NiveauForce::Fort,
                            format!(
                                "ATR ×{:.1} + MACD momentum haussier — tendance forte buy",
                                ratio
                            ),
                            atr[i],
                            c,
                    ...
                    if ml[i] < ms[i] && ml[i] < ml[i - 1] {
                        out.push(signal(
                            ...
                            NiveauForce::Fort,
                            format!(
                                "ATR ×{:.1} + MACD momentum baissier — tendance forte sell",
                                ratio
                            ),
                            atr[i],
                            c,
            ...
            if ema[i].is_finite() && ml[i].is_finite() && ms[i].is_finite() {
                ...
                if c > ema[i] && c_prev <= ema[i - 1] && macd_bull {
                    out.push(signal(
                        ...
                        NiveauForce::Moyen,
                        format!(
                            "Prix franchit l'EMA à hausse + MACD haussier — buy ({:.5})",
                            ema[i]
                        ),
                        ema[i],
                        c,
                ...
                if c < ema[i] && c_prev >= ema[i - 1] && macd_bear {
                    out.push(signal(
                        ...
                        NiveauForce::Moyen,
                        format!(
                            "Prix franchit l'EMA à baisse + MACD baissier — sell ({:.5})",
                            ema[i]
                        ),
                        ema[i],
                        c,
                ...
                && ms[i].is_finite()
            {
                ...
                if golden && ml[i] > ms[i] {
                    out.push(signal(
                        ...
                        NiveauForce::Fort,
                        format!(
                            "Golden Cross EMA + MACD aligné — confluece buy majeure ({:.5})",
                            ema[i]
                        ),
                        ema[i],
                        c,
                ...
                if death && ml[i] < ms[i] {
                    out.push(signal(
                        ...
                        NiveauForce::Fort,
                        format!(
                            "Death Cross EMA + MACD aligné — confluence sell majeure ({:.5})",
                            ema[i]
                        ),
                        ema[i],
                        c,
