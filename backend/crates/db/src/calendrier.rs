...
impl Database {
    ...
    /// L'instant présent est-il dans une FENÊTRE MACRO : à ± `minutes` d'une
    /// annonce High-impact du calendrier ? (décision owner 26/09 : les
    /// signaux — sauf straddle — sont pausés 30 min avant/après chaque
    /// annonce.) Remplace deux helpers jamais branchés (zéro dette).
    pub async fn fenetre_macro_active(&self, minutes: i64) -> Result<bool> {
        let events = self.lire_calendrier_cache(30 * 86_400).await?;
        let maintenant = chrono::Utc::now();
        Ok(events.iter().any(|ev| {
            if ev["impact"].as_str().unwrap_or("") != "High" {
                return false;
            }
            let date_str = ev["date_heure"].as_str().unwrap_or("");
            match chrono::DateTime::parse_from_rfc3339(date_str) {
                Ok(d) => (d.with_timezone(&chrono::Utc) - maintenant).num_minutes().abs() <= minutes,
                Err(_) => false,
            }
        }))
    }
