...
pub async fn lire_tous(pool: &SqlitePool) -> Result<Vec<AssetParams>> {
    let rows = sqlx::query(
        "SELECT asset, valeur_pips, sl_pips, pip_to_points, risque_pct, lot_min, lot_max, taille_pip
...
pub async fn sauvegarder(pool: &SqlitePool, params: &AssetParams) -> Result<()> {
    sqlx::query(
        "INSERT INTO asset_params (asset, valeur_pips, sl_pips, pip_to_points, risque_pct, lot_min, lot_max, taille_pip, maj_le)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
