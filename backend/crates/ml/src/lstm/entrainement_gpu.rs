...
// Sérialise les entraînements GPU : Ollama occupe ~22 GB VRAM sur RTX 3090,
// laissant ~2 GB libres. Un seul job à la fois évite le CUDA OOM.
static GPU_TRAIN_LOCK: OnceLock<std::sync::Mutex<()>> = OnceLock::new();
...
/// Le VarStore est sauvegardé au format `.pt` dans `chemin_pt`.
pub(crate) fn entrainer_sur_gpu(
    ...
    // Un seul job GPU à la fois — évite le CUDA OOM lors de retrains concurrents.
    // Poisoned mutex : on récupère le guard sans paniquer.
    let _gpu_guard = GPU_TRAIN_LOCK
    ...
    // Sauvegarde du VarStore au format .pt (chargé par LstmGpu::depuis_pt pour l'inférence)
    if let Err(e) = vs.save(chemin_pt) {
        tracing::warn!("LSTM GPU: échec sauvegarde .pt '{}': {}", chemin_pt, e);
    } else {
        tracing::info!("LSTM GPU: poids sauvegardés dans '{}'", chemin_pt);
    }
...
fn forward_net(l1: &nn::LSTM, l2: &nn::LSTM, l3: &nn::LSTM, fc: &nn::Linear, x: &Tensor) -> Tensor {
    let (h1, _) = l1.seq(x);
    let (h2, _) = l2.seq(&h1);
    let (h3, _) = l3.seq(&h2);
    let t = h3.size()[1];
...
fn extraire_par_couches(vars: &HashMap<String, Tensor>, _n_feat: usize) -> Result<PoidsCouches> {
    ...
    let sortie_poids: Vec<Vec<f64>> = (0..2)
        .map(|r| {
            (0..L3)
                .map(|c| fc_w.double_value(&[r as i64, c as i64]))
                .collect()
        })
        .collect();
...
fn fusionner_couche(vars: &HashMap<String, Tensor>, pfx: &str) -> Result<(Vec<f64>, Vec<f64>)> {
    ...
    let poids: Vec<f64> = (0..w.numel() as i64)
        .map(|i| w.double_value(&[i]))
        .collect();
    let biais: Vec<f64> = (0..b.numel() as i64)
        .map(|i| b.double_value(&[i]))
        .collect();
    Ok((poids, biais))
}
...
fn get_var(vars: &HashMap<String, Tensor>, name: &str) -> Result<Tensor> {
    ...
    Err(anyhow!(
        "Variable GPU '{}' introuvable. Disponibles: {:?}",
        name,
        keys
    ))
}
