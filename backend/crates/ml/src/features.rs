...
pub fn extraire_features(bougies: &[Candle]) -> Option<Vec<f64>> {
    ...
    let safe = |v: f64| if v.is_nan() || v.is_infinite() { 0.0 } else { v };
    ...
    f.push(safe(courant.close / bougies[i.saturating_sub(1)].close - 1.0)); // rendement
    let vol_moy: f64 = bougies[i.saturating_sub(20)..=i].iter().map(|b| b.volume).sum::<f64>() / 21.0;
    ...
    f.push(safe((courant.high - courant.close.max(courant.open)) / prix_ref)); // mèche haute
    f.push(safe((courant.close.min(courant.open) - courant.low) / prix_ref));  // mèche basse
    ...
    let pente_ema21 = if i >= 3 { safe(ema21[i] / ema21[i - 3].max(1e-10) - 1.0) } else { 0.0 };
    f.push(pente_ema21);
    ...
    let atr_moy: f64 = (1..=14).filter_map(|k| {
    ...
    f.push(if safe(atr14[i] / atr_moy.max(1e-10)) > 1.5 { 1.0 } else { 0.0 });
    ...
    // Croisements MACD
    f.push(if i > 0 && macd.ligne[i] > macd.signal[i] && macd.ligne[i-1] <= macd.signal[i-1] { 1.0 } else { 0.0 });
    f.push(if i > 0 && macd.ligne[i] < macd.signal[i] && macd.ligne[i-1] >= macd.signal[i-1] { 1.0 } else { 0.0 });
    ...
    for k in 1..=5 {
        ...
        let rdt = prev.map(|b| safe(prix_ref / b.close.max(1e-10) - 1.0)).unwrap_or(0.0);
        f.push(rdt);
    }
    // Momentum 10 et 20 périodes
    f.push(if i >= 10 { safe(prix_ref / bougies[i-10].close.max(1e-10) - 1.0) } else { 0.0 });
    f.push(if i >= 20 { safe(prix_ref / bougies[i-20].close.max(1e-10) - 1.0) } else { 0.0 });
    f.push(if i >= 30 { safe(prix_ref / bougies[i-30].close.max(1e-10) - 1.0) } else { 0.0 });

    // ─── 43-47 : Volume ──────────────────────────────────────────────────────
    // Volume des 5 dernières bougies normalisé
    for k in 1..=5 {
        let vol_prev = i.checked_sub(k).map(|idx| bougies[idx].volume).unwrap_or(0.0);
    ...
    let trois_haussiers = (1..=3).all(|k| {
        i.checked_sub(k).map(|idx| bougies[idx].close > bougies[idx].open).unwrap_or(false)
    });
