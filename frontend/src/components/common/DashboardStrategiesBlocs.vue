<template>
  <div class="h-full min-h-0 overflow-y-auto flex flex-col gap-2 pr-0.5">
    <div
      ...
      @click="ouvrir(b.id)"
    >
      <!-- En-tête : identité + état + les deux voix (ΣR distance · $) —
           survols en panneaux stylés (PopoverInfo, design info-bulle
           capital, P4 24/09). -->
      <div class="flex items-center gap-2 flex-wrap">
        <span class="text-lg leading-none">{{ b.icone }}</span>
        <span class="font-semibold text-white text-sm">{{ b.nom }}</span>
        <PopoverInfo :texte="texteEtat(b.etat)">
          <span
            class="text-[10px] font-semibold px-2 py-0.5 rounded-full border cursor-help"
        ...
        <div class="ml-auto flex items-center gap-1.5 text-[10px] font-semibold whitespace-nowrap">
          ...
          <PopoverInfo :texte="titreTelegram(b)">
            <button
          ...
          <PopoverInfo :texte="titleR(b)">
            <span class="px-1.5 py-0.5 rounded bg-white/10 font-mono font-bold cursor-help"
            ...
            >{{ rFormate(b.analyse?.r_distance_total ?? 0) }}</span>
          </PopoverInfo>
          <PopoverInfo v-if="b.capital" :texte="`Capital simulé — départ ${fmtDollars(b.capital.capital_depart)}, compose à chaque clôture (risque ${(b.capital.fraction_risque * 100).toFixed(b.capital.fraction_risque < 0.01 ? 1 : 0)} %/trade).\nLe lot de chaque trade se calcule sur ce capital.`">
            <span class="px-1.5 py-0.5 rounded bg-white/10 font-mono font-bold cursor-help"
            ...
            >{{ fmtDollars(b.capital.capital_actuel) }}</span>
          </PopoverInfo>
        <PopoverInfo v-if="b.id === 'rockets' && nbPositionsRocket > 0" :texte="titrePositionsRocket">
          <span class="px-1.5 py-0.5 rounded bg-purple-700/40 text-purple-200 font-mono font-bold cursor-help"
          >{{ nbPositionsRocket }} en cours 🚀</span>
        </PopoverInfo>
        <PopoverInfo v-else-if="(enCoursBloc[b.id] || []).length > 0" :texte="titreEnCours(b.id)">
          <span class="px-1.5 py-0.5 rounded bg-blue-500/30 text-blue-200 font-mono font-bold cursor-help"
          >{{ (enCoursBloc[b.id] || []).length }} en cours</span>
        </PopoverInfo>
        </div>
      </div>
...
/// (R, WR, histogramme) viennent de l'analyse — même source que
...
/// Info-bulle du badge R (23/09, enrichie P4 24/09) : R DISTANCE cumulée —
/// le niveau le plus lointain atteint par les trades. Juge la stratégie
/// (entrées + TP) ; le badge $ du capital juge le résultat réel, réglages
/// de sortie compris. P4 : la base (clôtures + fenêtre) vit ici — le badge
/// clôtures et le WR ont quitté l'en-tête (page Analyse).
function titleR(b: Bloc): string {
  const base = b.analyse?.nb_trades
    ? `\nBase vécue complète : ${b.analyse.nb_trades} clôtures, ${dateCourte(b.analyse.fenetre_debut)} → ${dateCourte(b.analyse.fenetre_fin)} · WR $ ${((b.analyse.taux_reussite ?? 0) * 100).toFixed(0)} %`
    : ''
  return `R distance cumulé : niveaux les plus lointains atteints (juge la stratégie — entrées et TP).${base}\nLe badge capital $ dit ce que ça a rapporté, réglages de sortie compris.`
}
...
/// Texte du panneau d'état (P4) : ce que chaque état FAIT réellement.
function texteEtat(etat: string): string {
  if (etat === 'Officielle') return 'Officielle — signaux émis ET Telegram actif (si la cloche l\'est).'
  if (etat === 'Observation') return 'Observation — signaux journalisés en base, silencieux : mesure sans bruit.'
  return 'Construction — aucun signal écrit, la stratégie se construit.'
}
  ...
  try {
    ...
    const complets = await Promise.allSettled(
      actives.map(async s => {
        ...
        // badge R, WR — exactement les mêmes nombres que le rapport
        // d'activité. Le /capital ne sert plus que la courbe.
        // P1-P2 (24/09) : camemberts et histogramme ont quitté la carte —
        // ils vivent dans les pages Analyse.
