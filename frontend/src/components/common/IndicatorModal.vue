      ...
      >
        ...
        <div class="relative bg-[#0f1629] border border-white/10 rounded-2xl p-6 w-72 shadow-2xl">
          ...
          <template v-if="indicateur === 'ema'">
            <label class="block text-xs text-white mb-1">Période</label>
            <input
            ...
            />
            <p class="text-[10px] text-white mt-1">Recommandé : 9, 20, 50, 200</p>
            <label class="block text-xs text-white mb-1 mt-3">Type de MA</label>
            <select
            ...
            <label class="block text-xs text-white mb-1 mt-3">Couleur</label>
            <div class="flex items-center gap-3">
              ...
              />
              <span class="text-xs text-white font-mono">{{ prefs.emaCouleur }}</span>
            </div>
          </template>
          ...
          <template v-else-if="indicateur === 'atr'">
            ...
            <p class="text-[10px] text-white mt-1">Standard : 14 (Wilder)</p>
            <label class="block text-xs text-white mb-1 mt-3">Couleur</label>
            <div class="flex items-center gap-3">
              <input type="color" v-model="prefs.atrCouleur"
                class="w-10 h-8 rounded cursor-pointer border border-white/15 bg-transparent" />
              <span class="text-xs text-white font-mono">{{ prefs.atrCouleur }}</span>
            </div>
          </template>
          ...
          <template v-else-if="indicateur === 'bollinger'">
            <div class="flex gap-2">
              ...
              <div class="flex-1">
                <label class="block text-xs text-white mb-1">Multiplicateur σ</label>
                <input type="number" min="0.1" max="5" step="0.1" v-model.number="prefs.bollingerStdDev"
                  class="w-full bg-white/10 border border-white/15 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-indigo-500/60 transition-colors" />
              </div>
            </div>
            <label class="block text-xs text-white mb-1 mt-3">Type de MA (base)</label>
            <select
            ...
            <div class="grid grid-cols-3 gap-2 mt-3">
              <div>
                <label class="block text-[10px] text-white mb-1">Bande haute</label>
                <div class="flex items-center gap-1.5">
                  ...
                  <span class="text-[10px] text-white font-mono truncate">{{ prefs.bollingerCouleurHaute }}</span>
                </div>
              </div>
              <div>
                <label class="block text-[10px] text-white mb-1">Basis</label>
                <div class="flex items-center gap-1.5">
                  <input type="color" v-model="prefs.bollingerCouleurMilieu"
                    class="w-8 h-7 rounded cursor-pointer border border-white/15 bg-transparent" />
                  <span class="text-[10px] text-white font-mono truncate">{{ prefs.bollingerCouleurMilieu }}</span>
                </div>
              </div>
              <div>
                <label class="block text-[10px] text-white mb-1">Bande basse</label>
                <div class="flex items-center gap-1.5">
                  <input type="color" v-model="prefs.bollingerCouleurBasse"
                    class="w-8 h-7 rounded cursor-pointer border border-white/15 bg-transparent" />
                  <span class="text-[10px] text-white font-mono truncate">{{ prefs.bollingerCouleurBasse }}</span>
                </div>
              </div>
            </div>
            <p class="text-[10px] text-white mt-2">Standard : 20 / 2.0 / SMA</p>
          </template>
          ...
          <template v-else-if="indicateur === 'macd'">
            <div class="flex gap-2">
              <div class="flex-1">
                <label class="block text-xs text-white mb-1">Rapide</label>
              ...
              <div class="flex-1">
                <label class="block text-xs text-white mb-1">Lente</label>
                <input type="number" min="2" max="200" v-model.number="prefs.macdLente"
                  class="w-full bg-white/10 border border-white/15 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-emerald-500/60 transition-colors" />
              </div>
            </div>
            <label class="block text-xs text-white mb-1 mt-3">Signal</label>
            <input type="number" min="2" max="50" v-model.number="prefs.macdSignal"
              class="w-full bg-white/10 border border-white/15 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-emerald-500/60 transition-colors" />
            <p class="text-[10px] text-white mt-1">Standard : 12 / 26 / 9</p>
          </template>
          ...
          <template v-else-if="indicateur === 'rsi'">
            ...
            />
            <p class="text-[10px] text-white mt-1">Recommandé : 14 (standard), 7 (scalping)</p>
            <div class="flex gap-2 mt-3">
              <div class="flex-1">
                <label class="block text-xs text-white mb-1">Surachat</label>
                <input
              ...
              <div class="flex-1">
                <label class="block text-xs text-white mb-1">Survente</label>
                <input
            ...
            <div class="flex items-center gap-3">
              <input
                ...
                class="w-10 h-8 rounded cursor-pointer border border-white/15 bg-transparent"
              />
              <span class="text-xs text-white font-mono">{{ prefs.rsiCouleur }}</span>
            </div>
          </template>

          <!-- Kasper Tendance (EMA Crossover MTF) -->
          <template v-else-if="indicateur === 'kasperTendance'">
            <label class="block text-xs text-white mb-1">Période EMA rapide</label>
            <input
            ...
            />
            <label class="block text-xs text-white mb-1 mt-3">Période EMA lente</label>
            <input
              type="number" min="2" max="500"
              v-model.number="prefs.kasperPeriodeLente"
              class="w-full bg-white/10 border border-white/15 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-sky-500/60 transition-colors"
            />
            <label class="block text-xs text-white mb-1 mt-3">Mode de calcul</label>
            <select
            ...
            <p class="text-[10px] text-white mt-2">Défauts : EMA 9 (rapide) / EMA 21 (lente)</p>
          </template>

          <div class="flex justify-end gap-2 mt-5">
            <button
              @click="$emit('fermer')"
              class="px-4 py-1.5 rounded-lg text-xs text-white hover:text-white border border-white/10 hover:border-white/20 transition-colors"
            >Annuler</button>
            <button
