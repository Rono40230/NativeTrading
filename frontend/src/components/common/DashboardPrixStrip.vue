    ...
    >
      ...
      <template v-else>
        <span class="text-sm font-bold mt-1">
          {{ a.prix !== null ? formatPrixAsset(a.prix) : '—' }}<span class="text-[9px] text-gray-500 font-normal ml-0.5">{{ deviseAsset(a.id) }}</span>
        </span>
        <span v-if="a.variation !== null" class="text-[10px] font-medium mt-0.5"
      ...
      >
        <div class="flex items-start justify-between mb-3">
          ...
          <div class="text-right">
            <span class="text-base font-bold text-white">{{ hoveredAsset.prix !== null ? formatPrixAsset(hoveredAsset.prix) : '—' }}</span>
...
const props = defineProps<{ assets: AssetAvecPrix[] }>()
