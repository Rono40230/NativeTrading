<template>
  <div class="absolute top-3 left-3 z-10 select-none" style="min-width: 148px;">
    <div class="bg-[#0a0e27]/90 border border-white/10 rounded-xl overflow-hidden shadow-2xl backdrop-blur-sm">
      ...
      <div class="flex items-center justify-between px-3 py-1.5 border-b border-white/10">
        <span class="text-[10px] font-semibold text-slate-300 uppercase tracking-wide">Tendance</span>
        <div class="flex items-center gap-1.5">
          <span class="text-[9px] px-1 py-0.5 rounded bg-blue-500/20 text-blue-300 font-mono font-semibold">{{ props.timeframe }}</span>
          <span class="text-[9px] text-slate-500">{{ props.maType.toUpperCase() }}({{ props.mmRapide }}/{{ props.mmLente }})</span>
        </div>
      </div>
      <!-- Lignes -->
...
const lignes = ref<LigneTendance[]>([])
const chargement = ref(false)
const erreur = ref<string | null>(null)

// Convertit le format chart (M1, H1…) vers le format retourné par l'API (1m, 1H…)
...
watch(
  () => [props.asset, props.timeframe, props.mmRapide, props.mmLente, props.maType],
  charger
)
</script>
