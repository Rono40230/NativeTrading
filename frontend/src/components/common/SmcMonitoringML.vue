<template>
  <div class="space-y-4">
    ...
    >
      ...
      <span class="text-orange-400/70">— Win rate &lt; 45% sur les 20 derniers trades. Recalibration en cours (&lt; 6h).</span>
    </div>
    ...
    <div v-if="calibration.length">
      <p :class="props.compact ? 'text-[10px]' : 'text-[11px]'" class="font-semibold uppercase tracking-wider text-gray-400 mb-2">Calibration par triplet</p>
    ...
    <div v-if="!props.compact && monitoring?.par_categorie?.length">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-xs">
          ...
          <tbody>
            <tr
              v-for="(p, i) in monitoring.par_categorie"
              :key="i"
              class="border-b border-white/5 hover:bg-white/5"
            >
              <td class="py-1.5 pr-3">
