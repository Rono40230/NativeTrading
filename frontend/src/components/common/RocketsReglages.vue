<template>
  <div class="space-y-6 p-2">
    ...
    <!-- Chargement -->
    <div v-if="chargement" class="text-white text-sm">Chargement de la configuration…</div>
    ...
    <template v-else-if="cfg">
      ...
      <div class="glass-param">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-baseline gap-2">
            ...
            <span class="text-xs text-white">Seuls les signaux avec un score ≥ {{ cfg.score_min }} sont sauvegardés.</span>
          </div>
      ...
      <div class="glass-param">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-baseline gap-2">
            ...
            <span class="text-xs text-white">Filtrer les signaux en zone de surachat extrême (RSI > {{ cfg.rsi_max }}).</span>
          </div>
      ...
      <div class="glass-param">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-baseline gap-2">
            ...
            <span class="text-xs text-white">Filtrer les signaux en zone de survente (RSI < {{ cfg.rsi_min }}). 0 = désactivé.</span>
          </div>
      ...
      <div class="glass-param">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-baseline gap-2">
            ...
            <span class="text-xs text-white">Volume actuel ≥ {{ cfg.ratio_volume_min }}× la moyenne des 20 dernières bougies.</span>
          </div>
      ...
      <div class="glass-param">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-baseline gap-2">
            ...
            <span class="text-xs text-white">Ignorer les paires avec moins de {{ formatVol(cfg.vol_marche_min) }} de volume journalier.</span>
          </div>
      ...
      <div class="glass-param">
        <div class="flex items-baseline gap-2 mb-3">
          ...
          <span class="text-xs text-white">Seules les phases cochées génèrent des signaux sauvegardés.</span>
        </div>
        <div class="flex gap-3">
          <label v-for="p in PHASES" :key="p.val" class="flex items-center gap-2 cursor-pointer select-none">
            ...
            />
            <span class="text-sm" :class="cfg.phases_actives.includes(p.val) ? 'text-white' : 'text-white'">{{ p.label }}</span>
          </label>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3 pt-2">
        ...
        <button
          class="px-4 py-2 rounded-lg border border-white/15 text-white hover:text-white text-sm transition-all"
          @click="reinitialiser"
