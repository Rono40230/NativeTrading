  ...
  <div class="flex flex-col" style="height: calc(100vh - 3rem);">
      ...
      <div class="flex gap-2 items-center">
        ...
        <button
          ...
          :class="{
            ...
            'bg-gray-700/50 text-gray-400 hover:bg-gray-600/50 line-through': anthropicStatut === 'ok' && !anthropicActif,
    ...
    <div class="flex-1 min-h-0 grid grid-cols-2 gap-4">
      ...
      <div ref="zoneChat" class="glass-card p-4 space-y-4 overflow-y-auto h-full min-h-0 flex flex-col">
        <div v-if="messages.length === 0" class="text-center text-gray-500 py-12 space-y-2 flex-1 flex flex-col items-center justify-center">
          <p class="text-3xl">🤖</p>
    ...
    <div v-if="!ollamaOk" class="glass-card p-5 border-yellow-500/30 bg-yellow-900/10 mt-3 flex-shrink-0">
      ...
      <pre class="text-xs text-gray-300 bg-black/30 p-3 rounded">ollama pull qwen2.5vl:7b
ollama serve</pre>
    </div>
  </div>
</template>
...
function renderCoachMd(text: string): string {
  return text
    ...
    .replace(/^- (.+)$/gm, '<span class="block pl-3 before:content-[\'-\'] before:mr-2 before:text-gray-500">$1</span>')
    .replace(/\n\n/g, '<br><br>')
    .replace(/\n/g, '<br>')
}
